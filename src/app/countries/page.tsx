...
export default function CountriesPage() {
    ...
    // Get unique regions (filter out empty strings)
    const regions = [...new Set(countries.map(c => c.region).filter(r => r && r.trim()))].sort();

    // Filter and sort countries
    let filteredCountries = selectedRegion
        ? countries.filter(c => c?.country && c.region === selectedRegion)
    ...
    const sortedCountries = [...filteredCountries].sort((a, b) => {
        // Safety check for undefined countries
        ...
        return (
            <div className="min-h-screen">
                ...
                <div className="flex items-center justify-center h-[calc(100vh-80px)]">
                    <LoadingSpinner size="xl" text="Loading countries..." />
                </div>
            </div>
        );
    }
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 py-4 sm:py-8">
                ...
                <div className="flex items-center gap-2 sm:gap-4 mb-4 text-sm overflow-x-auto pb-2 -mx-4 px-4 sm:mx-0 sm:px-0 sm:overflow-visible">
                    ...
                    {(['name', 'gdp', 'population', 'growth'] as const).map(key => (
                        <button
                            ...
                            className={`px-3 py-2 rounded-lg capitalize transition whitespace-nowrap touch-target ${sortBy === key
                ...
                <div className="flex gap-2 mb-4 sm:mb-6 overflow-x-auto pb-2 -mx-4 px-4 sm:mx-0 sm:px-0 sm:flex-wrap sm:overflow-visible">
                    <button
                        ...
                        className={`px-3 sm:px-4 py-2 rounded-full text-sm transition whitespace-nowrap flex-shrink-0 ${!selectedRegion
                    ...
                    {regions.map(region => (
                        <button
                            ...
                            className={`px-3 sm:px-4 py-2 rounded-full text-sm transition whitespace-nowrap flex-shrink-0 ${selectedRegion === region
                                ? 'bg-blue-500 text-white'
                                : 'bg-white text-slate-600 hover:bg-slate-50 border border-slate-200'
                                }`}
                        >
